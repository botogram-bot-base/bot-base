import redis                                            # Required to work with redis
from source.objects.config_parser import ConfigParser   # Configuration parser to get redis db info
from botogram import User as bUser                      # Get a botogram.User class and save it as bUser
from typing import Union                                # Needed for parameters and return hints
from datetime import datetime as dt                     # Needed to save the last activity
config = ConfigParser("data/configs/config.json")       # Load the config file and parse it
...
class User:
    ...
    def __init__(self, botogram_user: bUser = None, telegram_id: int = 0):
        ...
        ------
        ValueError
            If neither the botogram_user nor telegram_id is provided or if both are provided and don't match
        """
        ...
        if botogram_user and not telegram_id or botogram_user.id == telegram_id:     # If a botogram User is passed
            #                                                                          or both are passed and they match
            self.id = botogram_user.id                      # Save its id as an attribute
            self.redis_hash = f"user:{self.id}"             # Get it's redis hash from the id

            if not (self._get_redis_value("id")):            # If its user data is not present on redis
                ...
                self._set_redis_value("last_activity", self.last_activity)  # Set its last activity on redis
                self.state(self._state)                                  # Save the user state

            else:           # If the user is present on redis
                self.first_name = self._get_redis_value("first_name")       # Get its info
            ...
            if not(self._get_redis_value("id")):    # Check if it's present on redis
                raise ValueError("User not found in the redis database, cannot utilize the telegram id")
                # Can't get data from an id, so raise ValueError

            self.first_name = self._get_redis_value("first_name")       # Else get its data
            self.last_name = self._get_redis_value("last_name")
            self.username = self._get_redis_value("username")
            self.last_activity = self._get_redis_value("last_activity", float)
            self._state = self.state()
            ...
            raise ValueError("Both a Botogram User and a Telegram ID were passed, but they didn't match")
    ...
    def _set_redis_value(self, key: str, value: Union[str, int, float]) -> bool:
        ...
        -------
        bool
            True if the operation succeeded and a new entry was created, False if an existing key was altered
        """
    ...
    def state(self, new_state: str = "") -> Union[str, bool]:
        ...
        if not new_state:                               # If a new state wasn't passed
            return self._get_redis_value("state")       # Get the current one

        self._set_redis_value("state", new_state)       # Otherwise set the new state
